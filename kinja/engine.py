...
from kinja.util import (b64decode, b64encode, choice, hash,  # ,include_file
                        random_password)
...
def build(path: str, cluster: str, environment: str):
    ...
    env.globals['get_secret'] = lambda key: get_secret(key, path, cluster, environment)

    return env


def find_template_paths(path: str, cluster: str, environment: str):
    LOGGER.debug("finding template paths in %s for %s on %s", path, cluster, environment)
    ...
    if cluster is not None:
        try:
            ...
            if os.path.exists(cluster_template_path):
                LOGGER.debug('adding %s to template paths', cluster_template_path)
                template_paths.append(cluster_template_path)

            if environment is not None:
                environment_path = get_environment_path(environment, cluster_path)

                environment_template_path = os.path.join( environment_path, 'templates')

                if os.path.exists(environment_template_path):
                    LOGGER.debug('adding %s to template paths', environment_template_path)
