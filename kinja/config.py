...
def load_configuration(path: str, cluster: str, environment: str) -> dict:
    ...
    if cluster:
        cluster_config_path = os.path.join(path, 'clusters', cluster, 'config.yaml')

        if os.path.exists(cluster_config_path):
            config = merge(config, load_config_file(cluster_config_path))

        if environment:
            environment_config_path = os.path.join(path, 'clusters', cluster, 'environments', environment, 'config.yaml')

            if os.path.exists(environment_config_path):
                config = merge(config, load_config_file(environment_config_path))
